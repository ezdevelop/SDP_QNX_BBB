	...
	while (options && *options != '\0') {
		...
		switch (opt) {
			...
			case 12:
				...
				continue;
			case 13:
				chan_num = strtoul(value, 0, 0);
				if (*(options-3) == '=') {
					cspol = strtoul(options - 2, &options - 1, 0);
					if (!cspol)
						devlist[chan_num].cfg.mode |= SPI_MODE_CSPOL_HIGH;
				}
				continue;
			case 14:
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/spi/dm816x/omap3spi.c $ $Rev: 716134 $")
#endif
