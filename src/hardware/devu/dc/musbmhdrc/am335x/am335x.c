...
/*
 * am335x uses external DMA (CPPI 4.1), but DMA is not currently supported
 * for the USB Device Controller Driver
 */
void musb_dma_map( musb_dma_funcs_t * dma_funcs ) {
	dma_funcs->init             = NULL;
	dma_funcs->fini             = NULL;
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/devu/dc/musbmhdrc/am335x/am335x.c $ $Rev: 724084 $")
#endif
