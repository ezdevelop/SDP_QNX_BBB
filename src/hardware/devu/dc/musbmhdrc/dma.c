	...
	if ( ch_ctx->channel_active ) {
		...
		if ( ch_ctx->ep->dir == USB_ENDPOINT_IN ) {
			if ( ch_ctx->process_residual_required ) {
				...
				ep->req_xfer_len = HW_Read32( dc->IoBase, MUSB_DMA_COUNT(chidx) );
				HW_Write16Or( dc->IoBase, MUSB_TXCSR( ch_ctx->ep->num ), TXCSR_TXPKTRDY );
			}
		} 
	
		// calc how much data we sent...
		ep->bytes_xfered = ch_ctx->length - HW_Read32( dc->IoBase, MUSB_DMA_COUNT(chidx) );
		MUSB_SLOGF_DBG(dc, _SLOG_DEBUG1, "%s: epnum = 0x%x epdir = 0x%x bytes_xfered %d bytes, dmacount = 0x%x",__func__,ep->num, ep->dir, ep->bytes_xfered, HW_Read32( dc->IoBase, MUSB_DMA_COUNT(chidx) ) );	
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/devu/dc/musbmhdrc/dma.c $ $Rev: 680332 $")
#endif
